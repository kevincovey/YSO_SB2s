...
## enable processing of YSO catalogs generated by APOGEE_ID.py (either the DR14 or pre-DR15 versions)
#DR14Stats = 'YSO_StatsCatalog.csv' #actually preDR15, but use the same code for convenience
DR14Stats = 'YSO_DR14_StatsCatalog.csv'  
...
#df.to_csv('YSO_SmallR_LargeXR.csv')
df.to_csv('YSO_DR14_SmallR_LargeXR.csv')
...
MainResults = 'YSO_DR14_SmallR_LargeXR.csv'
#MainResults = 'YSO_SmallR_LargeXR.csv'
...
#select elements matching our criteria for SB2 status
for i in range(len(dr14_R101)):
    if ( ( (dr14_R101[i] < 0.85 and dr14_ratio1[i] > 0.06 and dr14_ratio1[i] < 0.2) or (dr14_R51[i] < 0.85 and dr14_ratio2[i] > 0.05 and dr14_ratio2[i] < 0.2) ) and dr14_xr[i] > 0.5 and dr14_xr[i] < 1.75 and dr14_peak_value[i] > -0.5 and dr14_R51[i] > 0.25 and dr14_R101[i] > 0.22):
        print(dr14_apogeeid[i], ' is an SB2!')
        sb2_locids.append(dr14_locationid[i])
        sb2_apogeeids.append(dr14_apogeeid[i])
        sb2_R51s.append(dr14_R51[i])
        sb2_R101s.append(dr14_R101[i])
        sb2_R151s.append(dr14_R151[i])
        sb2_xrs.append(dr14_xr[i])
        sb2_peak_values.append(dr14_peak_value[i])
        
sb2s = pd.DataFrame()
sb2s['LocationID'] = sb2_locids
sb2s['ApogeeID'] = sb2_apogeeids
#add HJD info to output table
sb2s['log(R51)'] = sb2_R51s
sb2s['log(R101)'] = sb2_R101s
sb2s['log(R151)'] = sb2_R151s
sb2s['log(xr)'] = sb2_xrs
sb2s['Peak_value'] = sb2_peak_values
#sb2s.to_csv('YSO_SB2s.csv')

sb2s.to_csv('YSO_DR14_SB2s.csv')
